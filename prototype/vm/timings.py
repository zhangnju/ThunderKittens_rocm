...
def save_gantt_chart(Timings, Instructions, name=None, verbose=False):
    ...
    timing_events = {
        ...
        126: {'marker': 'P', 'color': 'black', 'label': 'Start of consumer finish write', 'size': 32}
    }
    ...
    for proc in range(Timings.shape[0]):
        for instr in range(Timings.shape[1]):
            ...
            if Timings[proc, instr, 0].item() > 0:
                ...
                end = timings_us[proc, instr, 127].item() if Timings[proc, instr, 127].item() > 0 else start
                duration = end - start
