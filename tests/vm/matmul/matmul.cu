...
template<typename config=config> struct MatmulOp {
    ...
    __device__ static inline int get_inputs_arrived_phasebit(state<config> &s, int id) {
        return s.instruction_state().phasebits[id];
    }
    __device__ static inline int get_inputs_finished_phasebit(state<config> &s, int id) {
        return s.instruction_state().phasebits[id+PIPELINE_STAGES];
    }
    __device__ static inline int get_outputs_arrived_phasebit(state<config> &s, int id) {
        return s.instruction_state().phasebits[id+PIPELINE_STAGES*2];
    }
    __device__ static inline int get_outputs_shared_phasebit(state<config> &s, int id) {
        return s.instruction_state().phasebits[id+PIPELINE_STAGES*2+4];
    }

    __device__ static inline void set_inputs_arrived_phasebit(state<config> &s, int id, int phasebit) {
        s.instruction_state().phasebits[id] = phasebit;
    }
    __device__ static inline void set_inputs_finished_phasebit(state<config> &s, int id, int phasebit) {
        s.instruction_state().phasebits[id+PIPELINE_STAGES] = phasebit;
    }
    __device__ static inline void set_outputs_arrived_phasebit(state<config> &s, int id, int phasebit) {
        s.instruction_state().phasebits[id+PIPELINE_STAGES*2] = phasebit;
    }
    __device__ static inline void set_outputs_shared_phasebit(state<config> &s, int id, int phasebit) {
        s.instruction_state().phasebits[id+PIPELINE_STAGES*2+4] = phasebit;
    }
    ...
    struct loader {
        static __device__ void run(const globals &g, state<config> &s) {
            ...
            for(int i = 0; i < inst.iters; i++, pipeline_stage=ring_advance<PIPELINE_STAGES>(pipeline_stage)) {
                ...
                // if (blockIdx.x == 0 && laneid() == 0) printf("Loader inst %d, i = %d, phasebit = %d\n", s.instruction_index, i, phasebits[pipeline_stage]);
                if (i >= PIPELINE_STAGES) {
                    wait(inputs_finished(s, pipeline_stage), phasebits[pipeline_stage]);  
                    phasebits[pipeline_stage] = 1 - phasebits[pipeline_stage];
                } else {
                    phasebits[pipeline_stage] = get_inputs_finished_phasebit(s, pipeline_stage);
                }
            ...
            if (laneid() == 31) {
                for(int i = 0; i < PIPELINE_STAGES; i++)
                    set_inputs_finished_phasebit(s, i, phasebits[i]);
            }
        }
    };
    struct launcher { // launches mma's
        ...
        static __device__ void run(const globals &g, state<config> &s) {
            ...
            int phasebits[3];
            for (int i = 0; i < 3; ++i)
                phasebits[i] = get_inputs_arrived_phasebit(s, i);
            wait(inputs_arrived(s, pipeline_stage), phasebits[pipeline_stage]);
            phasebits[pipeline_stage] = 1 - phasebits[pipeline_stage];
            s.wait_tensor_ready();
            ...
            for(int i = 1; i < inst.iters-1; i++, pipeline_stage=ring_advance<PIPELINE_STAGES>(pipeline_stage)) {
                wait(inputs_arrived(s, pipeline_stage), phasebits[pipeline_stage]);
                phasebits[pipeline_stage] = 1 - phasebits[pipeline_stage];
            ...
            if(laneid() == 0) {
                for(int i = 0; i < PIPELINE_STAGES; i++)
                    set_inputs_arrived_phasebit(s, i, phasebits[i]);
            }
        }
    };
    struct consumer {
        static __device__ void run(const globals &g, state<config> &s) {
            ...
            int phasebits[4];
            for(int i = 0; i < 4; i++)
                phasebits[i] = get_outputs_arrived_phasebit(s, i);
            wait(outputs_arrived(s, groupid), phasebits[groupid]);
            phasebits[groupid] = 1 - phasebits[groupid];
            ...
            if (warpgroup::laneid() == 0) {
                set_outputs_arrived_phasebit(s, groupid, phasebits[groupid]);
            }
        }
    };
    struct storer {
        ...
        static __device__ void run(const globals &g, state<config> &s) {
            parsed_instruction inst{s};
            if(laneid() < 4) {
                int phasebit = get_outputs_shared_phasebit(s, laneid());
                wait(outputs_shared(s, laneid()), phasebit);
                ...
                set_outputs_shared_phasebit(s, laneid(), 1 - phasebit);
            }
            warp::sync();
