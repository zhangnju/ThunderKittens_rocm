...
int run_benchmark(size_t M, size_t N, size_t K) {
    ...
    // store timings and flops in vectors
    std::vector<double> times;
    std::vector<double> tflops_vec;
    std::vector<int> sm_counts;

    // Launch kernel
    dim3 block(NUM_THREADS);

    for (int sm_count = 2; sm_count <= 148; sm_count+=2) {
        sm_counts.push_back(sm_count);
        dim3 grid(sm_count, 1);
        ...
        std::chrono::duration<double> time_diff = std::chrono::duration<double>::max();
        ...
        for(int i = 0; i < ITERS; i++) {
            ...
            time_diff = std::min(time_diff, std::chrono::duration<double>(end - start));
            // wait 0.1 s
        ...
        // Calculate duration
        double useconds = time_diff.count() * 1e6;
        ...
        times.push_back(useconds);
        tflops_vec.push_back(tflops);
    }

    // save results to csv
    std::ofstream outfile("matmul_sm_sweep_results_min_time.csv");
    outfile << "SM_Count,Time(us),TFLOPs\n";
    for (size_t i = 0; i < times.size(); i++) {
        outfile << sm_counts[i] << "," << times[i] << "," << tflops_vec[i] << "\n";
    }
    outfile.close();
