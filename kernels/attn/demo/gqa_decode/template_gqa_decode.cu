...
// 16 HQ to 2 HKV
static constexpr int D = 128, Dd2 = D/2, NUM_ROWS = 32, PAGE_SIZE = 256, HKV = 2;
using qo_tile             = st_bf<64, D>;
using q_global            = kittens::gl<bf16, -1, -1, -1, D, qo_tile>; // B * R * H * D
using kcache_tile         = st_bf<NUM_ROWS, D>;
using vcache_tile         = st_bf<NUM_ROWS, D>; // we need the v_tile for later
using vcache_tile2        = st_bf<NUM_ROWS, Dd2>; // we need the v_tile for later
using kcache_global       = kittens::gl<bf16, -1, -1, PAGE_SIZE, D, kcache_tile>; // #page * pagesize * H_K * D
using vcache_global       = kittens::gl<bf16, -1, -1, PAGE_SIZE, D, vcache_tile>; // #page * pagesize * H_K * D
...
using o_tile_fl           = st_fl<16, D>;
using o_global            = kittens::gl<bf16, -1, -1, -1, D, st_bf<16, Dd2>, st_bf<16, D/8>>; // B * NEWTOKENS * H * D

using o_scratch_global    = kittens::gl<float, -1, -1, 16, D, st_fl<16, D/8>, st_fl<16,64>>; // For partial O's
...
struct partial_layout {
    ...
    struct scratch_block { qo_tile qo; st_bf<64, kcache_tile::rows> att_block; sv_fl<64> max_vec, norm_vec; };
    struct finish_block { st_fl<16, Dd2> o[4][2]; sv_fl<16> lvec[4]; };
    struct common_state {
        int uid;
        location dst;
        int q_batch_idx;
...
struct partial_template {
    ...
    struct producer {
        ...
        __device__ static inline void load(producer_load_args<layout> args) {
            ...
            if(warpgroup::warpid() == 0) {
                ...
                // cache shape is # pages, page size, HKV, D
                // TODO: this needs to change based on the group
                tma::load_async(args.input.kcache, args.globals.K_cache, { next_page_id, within_page_idx, 0, 0}, args.inputs_arrived);
                tma::load_async(args.input.vcache, args.globals.V_cache, { next_page_id, within_page_idx, 0, 0}, args.inputs_arrived);
    ...
    struct consumer {
        __device__ static inline void setup(consumer_setup_args<layout> args) {
            ...
            auto qst = subtile_inplace<16, Dd2>(args.scratch.qo, {warpgroup::warpid(), warpgroup::groupid()});
            load_async(qst, args.globals.Q, {args.common.q_batch_idx, args.common.q_seq_idx + warpgroup::warpid(), 0, warpgroup::groupid()});
        ...
        template<bool do_right_fill> __device__ static inline void internal_compute(consumer_compute_args<layout> args) {
            ...
            if(warpgroup::groupid() == 0) {
                ...
                warpgroup::mm_ABt(att_block_fp32, args.scratch.qo, args.input.kcache);
                ...
                if constexpr (do_right_fill) { // need to mask out a bunch of entries in the last page
                    // TODO: this needs to change based on the group logic
        ...
        __device__ static inline void finish(consumer_finish_args<layout> args) {
            ...
            if(args.common.dst.batch_idx >= 0) { // batch is meaningful
                auto &o_smem = reinterpret_cast<st_bf<16, Dd2>&>(args.finish.o[warpgroup::warpid()][warpgroup::groupid()]);
...
struct reduction_layout {
    using globals = config::globals;
    struct input_block   { st_fl<16, D/8> o[8]; sv_fl<16> lvec; sv_fl<16> padding[15]; };
    struct scratch_block { st_fl<16, D/8> o[8]; sv_fl<16> lvec; semaphore producer_block; }; // used both for setup load and finish store
    struct common_state {
        int uid;
...
struct reduction_template {
    ...
    struct consumer {
        ...
        __device__ static inline void finish(consumer_finish_args<layout> args) {
            ...
            if(args.common.dst.batch_idx >= 0) {
                auto &o_smem = reinterpret_cast<st_bf<16, D/8>&>(args.scratch.o[group<8>::warpid()]);
                store(o_smem, args.state.o);
                __syncwarp();
...
PYBIND11_MODULE(gqa_decode, m) {
    ...
    kittens::py::bind_kernel<interpreter::kernel<config, partial_template, reduction_template>>(m, "gqa_decode",
