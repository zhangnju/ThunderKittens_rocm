...
struct partial_layout {
    ...
    struct scratch_block { qrot_tile qrot; qvo_tile qvo; st_bf<64, kcache_tile::rows> att_block; sv_fl<64> max_vec, norm_vec; kittens::semaphore matmul_finished;};
...
struct partial_template {
    ...
    __device__ static inline void common_setup(common_setup_args<layout> args) {
        ...
        // if (warpgroup::groupid() == 0 && warpgroup::warpid() == 0 && laneid() == 0) printf("Launching partial for gridx=%d, gridy=%d, gridz=%d, uid=%d, dst[0]=%d, dst[1]=%d, q_batch_idx=%d, q_seq_idx=%d, num_iters=%d, start_pos=%d, end_pos=%d, length=%d\n", blockIdx.x, blockIdx.y, blockIdx.z, args.common.uid, args.common.dst.batch_idx, args.common.dst.seq_idx, args.common.q_batch_idx, args.common.q_seq_idx, args.num_iters, args.common.start_pos, args.common.end_pos, args.common.length);
    }
    struct producer {
    ...
    struct consumer {
        __device__ static inline void setup(consumer_setup_args<layout> args) {
            ...
            if (group<8>::laneid() == 0) {
                init_semaphore(args.scratch.matmul_finished, 2);
            }

            using o_tt_tile = tt<float, 64, QVO_Dd2>;
            o_tt_tile o_tt = args.tensor_alloc.template allocate<o_tt_tile>(0, warpgroup::groupid()*QVO_Dd2);

            rt_fl<16, QVO_Dd2> o;
            warp::zero(o); // normalize o_reg before mma

            warpgroup::store_async(o_tt, o);
            tensor_store_wait();

            warpgroup::sync(warpgroup::groupid()+8);
        ...
        template<bool do_right_fill> __device__ static inline void internal_compute(consumer_compute_args<layout> args) {
            // 1.44269504089f is from warp::exp2
            ...
            if(warpgroup::groupid() == 0) {
                // A = Q @ K.T
                using attn_tt_tile = tt<float, 64, kcache_tile::rows>;
                attn_tt_tile att_block_fp32_tt = args.tensor_alloc.template allocate<attn_tt_tile>(1, 0);

                warpgroup::mm_ABt(att_block_fp32_tt, args.scratch.qrot, args.input.kcache, args.scratch.matmul_finished);
                warpgroup::mma_ABt(att_block_fp32_tt, args.scratch.qvo, args.input.vcache, args.scratch.matmul_finished);
                ...
                kittens::wait(args.scratch.matmul_finished, 0);
                rt_fl<16, kcache_tile::rows> att_block_fp32_rt;
                warpgroup::load_async(att_block_fp32_rt, att_block_fp32_tt);

                // softmax
                if constexpr (do_right_fill) { // need to mask out a bunch of entries in the last page
                    // if (warpgroup::warpid() == 0 && laneid() == 0) printf("DOING RIGHT FILL. GRIDx=%d, GRIDy=%d, GRIDz=%d, warpgroup=%d, warpid=%d, laneid=%d, iter=%d\n", blockIdx.x, blockIdx.y, blockIdx.z, warpgroup::groupid(), warpgroup::warpid(), warpgroup::laneid(), args.iter);
                ...
                // Update max_vec and max_vec_scaled
                warp::row_max(local_max_vec, att_block_fp32_rt, local_max_vec);
                warp::mul(max_vec_scaled, local_max_vec, SOFTMAX_TEMPERATURE);

                // Scale att_block and subtract max
                warp::mul(att_block_fp32_rt, att_block_fp32_rt, SOFTMAX_TEMPERATURE);
                warp::sub_row(att_block_fp32_rt, att_block_fp32_rt, max_vec_scaled);

                // Exponentiate
                warp::exp2(att_block_fp32_rt, att_block_fp32_rt);
                ...
                warp::row_sum(local_norm_vec, att_block_fp32_rt, local_norm_vec);

                // Store att_block
                warpgroup::store(args.scratch.att_block, att_block_fp32_rt);
            ...
            // Load o, scale by scratch.max_vec, and store back into tensor memory
            rt_fl<16, QVO_Dd2> o;
            warpgroup::load_async(o, o_tt);
            warp::mul_row(o, o, max_vec_last_scaled);
            warpgroup::store_async(o_tt, o);
            tensor_store_wait();
            warpgroup::sync(warpgroup::groupid()+8);
            ...
            warpgroup::mma_AB(o_tt, args.scratch.att_block, v_smem[warpgroup::groupid()], args.scratch.matmul_finished);

            // Update max_vec and norm_vec states
            warp::copy(args.state.max_vec, local_max_vec);
            warp::copy(args.state.norm_vec, local_norm_vec);

            // Done!
            kittens::wait(args.scratch.matmul_finished, 1);
            if(warpgroup::laneid() == 0) arrive(args.inputs_finished, WARPGROUP_WARPS);
        }
        
        __device__ static inline void compute(consumer_compute_args<layout> args) {
            if (args.common.start_pos + NUM_ROWS * (args.iter + 1) >= args.common.length) internal_compute<true>(args);
            else internal_compute<false>(args);
        }
        
        __device__ static inline void finish(consumer_finish_args<layout> args) {
            if (group<8>::laneid() == 0) invalidate_semaphore(args.scratch.matmul_finished);

            col_vec<rt_fl<16, kcache_tile::rows>> local_max_vec, local_norm_vec;

            // Copy norm_vec and max_vec states from shared memory
            warp::copy(local_norm_vec, args.state.norm_vec);
            warp::copy(local_max_vec, args.state.max_vec);

            // Load o from tensor memory
            using o_tt_tile = tt<float, 64, QVO_Dd2>;
            o_tt_tile o_tt = args.tensor_alloc.template allocate<o_tt_tile>(0, warpgroup::groupid()*QVO_Dd2);
            rt_fl<16, QVO_Dd2> o;
            warpgroup::load_async(o, o_tt);
            ...
            // Normalize o
            warp::div_row(o, o, local_norm_vec);

            if(args.common.dst.batch_idx >= 0) { // final step -- write to O
            ...
            else { // future reductions will be needed -- write to O_scratch and Lvec_scratch
                if(warpgroup::groupid() == 0) {
                    // l_vec = log2(norm_vec) + max_vec
                    warp::mul(local_max_vec, local_max_vec, args.globals.Softmax_scale * 1.44269504089f);
                    warp::log2(local_norm_vec, local_norm_vec);
                    warp::add(local_norm_vec, local_norm_vec, local_max_vec);
                ...
                // Store o
                warp::store(args.finish.o[warpgroup::warpid()][warpgroup::groupid()], o);
                __syncwarp();
...
struct reduction_template {
        ...
        __device__ static inline void load(producer_load_args<layout> args) {
            ...
            if(warpgroup::warpid() == args.iter%4) {
                ...
                if(laneid() == 0) while(*(volatile int*)&args.globals.semaphore[{load_uid, args.common.dst.seq_idx}] != args.globals.tic) {
                    ...
                    if(current_clock - start_clock > cycle_trigger) {
                        printf("Block %d, thread %d, iter %d, uid %d, load uid %d, dst {%d, %d}\n", blockIdx.x, threadIdx.x, args.iter, args.common.uid, load_uid, args.common.dst.batch_idx, args.common.dst.seq_idx);
                        cycle_trigger *= 100000;
                    }
                }
                asm volatile("fence.sc.gpu;\n");
                __syncwarp();
    ...
    struct consumer {
        __device__ static inline void setup(consumer_setup_args<layout> args) {
            ...
            uint64_t start_clock = clock64(), cycle_trigger = 1000000;
            if(laneid() == 0) while(*(volatile int*)&args.globals.semaphore[{args.common.src_uid, args.common.dst.seq_idx}] != args.globals.tic) {
                uint64_t current_clock = clock64();
                if(current_clock - start_clock > cycle_trigger) {
                    printf("Block %d, thread %d, setup, uid %d, src uid %d, dst {%d, %d}\n", blockIdx.x, threadIdx.x, args.common.uid, args.common.src_uid, args.common.dst.batch_idx, args.common.dst.seq_idx);
                    cycle_trigger *= 100000;
                }
            }
            asm volatile("fence.sc.gpu;\n");
