...
template<typename config, typename globals, bool is_producer, typename... ops>
struct dispatch_ops {
    __device__ static inline void run(int opcode, const globals &g, kittens_virtual_machine_state<config> &kvms) {} // do nothing, base case
};
template<typename config, typename globals, bool is_producer, typename op, typename... ops>
struct dispatch_ops<config, globals, is_producer, op, ops...> {
    __device__ static inline void run(int opcode, const globals &g, kittens_virtual_machine_state<config> &kvms) {
        if(opcode == op::opcode) run_op<config, globals, is_producer, op>(g, kvms);
        else dispatch_ops<config, globals, is_producer, ops...>::run(opcode, g, kvms);
    }
};


template<typename config, typename globals> __device__ void inline load_instructions(int *instruction, int task_iter, const globals &g, kittens::semaphore &bar) {
    if(laneid() == 0) {
...
template<typename config, typename globals> __device__ void inline write_timings(int *timings, int task_iter, const globals &g) {
    if(laneid() == 0) {
