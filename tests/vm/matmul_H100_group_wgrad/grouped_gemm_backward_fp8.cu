...
template<typename config=config> struct GroupedGemmBackwardOp {
...
PYBIND11_MODULE(grouped_gemm_backward_fp8, m) {
    m.doc() = "grouped_gemm_backward_fp8 python module";
    m.def("grouped_gemm_backward_fp8", [](
        ...
        cudaFuncSetAttribute(kvm<config, globals, GroupedGemmBackwardOp<config>>, cudaFuncAttributeMaxDynamicSharedMemorySize, __dynamic_shared_memory__);
        kvm<config, globals, GroupedGemmBackwardOp<config>><<<__g__.grid(), __g__.block(), __dynamic_shared_memory__, raw_stream>>>(__g__);
    });
}
